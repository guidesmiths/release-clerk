...
module.exports = function getCommits(ctx, cb) {
    ...
    if (ctx.release.type === 'update' || ctx.release.type === 're-release') {
    ...
    ctx.github.repos.getCommits({
        ...
        for (var i = 0; i < data.length && data[i].sha !== sha1; i++) {
            ctx.commits.push({
                ...
                date: moment(_.get(data[i], 'commit.author.date') || _.get(data[i], 'commit.committer.date'), 'YYYY-MM-DD HH:mm Z').format('ddd Do MMM YYYY HH:mm:ss Z')
            })
        }
